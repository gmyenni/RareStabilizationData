...
plot(NA,NA,xlim=c(0,1),ylim=range(rates,na.rm=T),xlab="Relative Abundance",ylab=expression( paste('Growth Rate (log  ', lambda,')')) )
...
mtext(side=3,paste('pattern=',round(pattern$coefficients[2,1],3),'p-val=',round(pattern$coefficients[2,4],3),sep=" "),adj=0,line=0.5,cex=0.75)
...
#plot null pattern
#hist(null_pattern,breaks=20,xlim=c(min(min(null_pattern),pattern$coefficients[2,1]),max(max(null_pattern),pattern$coefficients[2,1]+.2)),main="Histogram of null pattern")
