...
p.val=length(which(null_pattern<pattern$estimate))/length(null_pattern)
...
#plot data and fitted negative frequency dependence
  ...
  mtext(side=3,paste('pattern=',round(pattern$estimate,3),'p-val=',round(length(which(null_pattern<pattern$estimate))/length(null_pattern),3),sep=" "),adj=0,line=0.5,cex=0.75)

#plot relationship between frequency and strength of stabilization
  plot(results2$intercept,-as.numeric(results2$slope),log='xy',pch=19,bty='l',xlab="Equilibrium Frequency",ylab='Stabilization',main="Observed pattern")
 #identify outliers
...
#plot null pattern
hist(null_pattern,breaks=20,xlim=c(min(min(null_pattern),pattern$estimate),max(max(null_pattern),pattern$estimate)+.2),main="Histogram of null pattern")
abline(v=pattern$estimate,col='red',lwd=2)

#save results
  parcel=list(results=results,pattern=pattern,p.val=p.val)
  return(parcel)
