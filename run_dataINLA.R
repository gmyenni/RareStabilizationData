...
portalrod_out=stabilINLA(portalE,c("Dipodomys.merriami","Dipodomys.ordii","Dipodomys.spectabilis","Chaetodipus.baileyi","Peromyscus.eremicus","Perognathus.flavus","Peromyscus.maniculatus","Chaetodipus.penicillatus","Reithrodomtomys.montanus"),"Portal Rodents")

#sevilleta
pgrassRA=read.table("./Mammals/sevilleta/5pgrass.txt",header=T)
pgrassE=read.table("./Mammals/sevilleta/5pgrass_E.txt",header=T)
pgrass_out=stabilINLA(pgrassE,names(pgrassE)[-(1:3)],"Sevilleta 5-Points Grass Rodents")
...
plarrea_out=stabilINLA(plarreaE,names(plarreaE)[-(1:3)],"Sevilleta 5-Points Larrea Rodents")
...
rslarrea_out=stabilINLA(rslarreaE,names(rslarreaE)[-(1:3)],"Sevilleta Rio Salado Larrea Rodents")

#Konza
konza=read.table("./Mammals/Konza_Abund.txt",header=T)
konza_out=stabilINLA(konza,names(konza)[-(1:3)],"Konza Rodents")
...
hubbard_out=stabilINLA(hubbard,names(hubbard)[-(1:3)],"Hubbard Brook")

#White Mountain NF
whitemt=read.table("./Birds/whitemountain.txt",header=T)
whitemt_out=stabilINLA(whitemt,names(whitemt)[-(1:3)],"White Mountain NF Birds")

#Redvers
redvers=read.csv("./Birds/redvers.csv",header=T)
redvers_out=stabilINLA(redvers,names(redvers)[-(1:3)],"Redvers Waterfowl")

#Skokholm
skokholm=read.table("./Birds/skokholm.txt",header=T)
skokholm_out=stabilINLA(skokholm,names(skokholm)[-(1:3)],"Skokholm Birds")

skokholm_water=read.table("./Birds/skokholm_water.txt",header=T)
skokholm_water_out=stabilINLA(skokholm_water,names(skokholm_water)[-(1:3)],"Skokholm Waterfowl")

skokholm_carrion=read.table("./Birds/skokholm_carrion.txt",header=T)
skokholm_carrion_out=stabilINLA(skokholm_carrion,names(skokholm_carrion)[-(1:3)],"Skokholm Carrion Birds")

#Konza LTER
konza_waterfowl=read.table("./Birds/konza_waterfowl.txt",header=T)
konza_waterfowl_out=stabilINLA(konza_waterfowl,names(konza_waterfowl)[-(1:3)],"Konza Waterfowl")

konza_raptors=read.table("./Birds/konza_raptors.txt",header=T)
konza_raptors_out=stabilINLA(konza_raptors,names(konza_raptors)[-(1:3)],"Konza Raptors")

konza_birds=read.table("./Birds/konza_birds.txt",header=T)
konza_birds_out=stabilINLA(konza_birds,names(konza_birds)[-(1:3)],"Konza Birds")

konza_game=read.table("./Birds/konza_gamebirds.txt",header=T)
konza_game_out=stabilINLA(konza_game,names(konza_game)[-(1:3)],"Konza Game Birds")

konza_songbirds=read.table("./Birds/konza_songbirds.txt",header=T)
konza_song_out=stabilINLA(konza_songbirds,names(konza_songbirds)[-(1:3)],"Konza Songbirds")

#Texas Migratory Birds
texas=read.table("./Birds/texas.txt",header=T)
texas_out=stabilINLA(texas,names(texas)[-(1:3)],"Texas Migratory Birds")

#Eastern Wood
easternwood=read.table("./Birds/easternwood.txt",header=T)
easternwood_out=stabilINLA(easternwood,names(easternwood)[-(1:3)],"Eastern Wood Breeding Birds")

#Pawnee Birds
pawnee_birds=read.table("./Birds/pawnee_birds.txt",header=T)
pawnee_birds_out=stabilINLA(pawnee_birds,names(pawnee_birds)[-(1:3)],"Pawnee Breeding Birds")

#Pawnee Raptors
pawnee_raptors=read.table("./Birds/pawnee_raptors.txt",header=T)
pawnee_raptors_out=stabilINLA(pawnee_raptors,names(pawnee_raptors)[-(1:3)],"Pawnee Raptors")

#Mountain Bird Watch
mountainbird=read.csv("./Birds/mountainbird.csv",header=T)
mountainbird_out=stabilINLA(mountainbird,names(mountainbird)[-(1:3)],"Mountain Bird Watch")

#Mountain Bird Watch - Maine
mountainbird_maine=read.table("./Birds/mountainbird_maine.txt",header=T)
mountainbird_maine_out=stabilINLA(mountainbird_maine,names(mountainbird_maine)[-(1:3)],"Maine Mountain Passerines")

#Mountain Bird Watch - New York
mountainbirdny=read.table("./Birds/mountainbird_ny.txt",header=T)
mountainbirdny_out=stabilINLA(mountainbirdny,names(mountainbirdny)[-(1:3)],"New York Mountain Passerines")

#Mountain Bird Watch - Green Mountains
mountainbirdvt=read.table("./Birds/mountainbird_vt.txt",header=T)
mountainbirdvt_out=stabilINLA(mountainbirdvt,names(mountainbirdvt)[-(1:3)],"Green Mountains Passerines")

#Luquillo Birds
elverde=read.table("./Birds/elverde.txt",header=T)
elverde_out=stabilINLA(elverde,names(elverde)[-(1:3)],"Luquillo Birds")

#################################REPTILES############################################################
#Luquillo
luquillo=read.table("./Herps/luquillo.txt",header=T)
luquillo_out=stabilINLA(luquillo,names(luquillo)[-(1:3)],"Luquillo Towers")

#Bold Park
boldpark=read.table("./Herps/boldpark.txt",header=T)
boldpark_out=stabilINLA(boldpark,names(boldpark)[-(1:3)],"Bold Park Herps")

boldparksnakes=read.table("./Herps/boldparksnakes.txt",header=T)
boldparksnakes_out=stabilINLA(boldparksnakes,names(boldparksnakes)[-(1:3)],"Bold Park Snakes")

boldparklizards=read.table("./Herps/boldparklizards.txt",header=T)
boldparklizards_out=stabilINLA(boldparklizards,names(boldparklizards)[-(1:3)],"Bold Park Lizards")

#Cowley County
cowleycounty=read.table("./Herps/cowleycounty.txt",header=T)
cowleycounty_out=stabilINLA(cowleycounty,names(cowleycounty)[-(1:3)],"Cowley County Herps")

cowleysnakes=read.table("./Herps/cowleysnakes.txt",header=T)
cowleysnakes_out=stabilINLA(cowleysnakes,names(cowleysnakes)[-(1:3)],"Cowley County Snakes")

cowleylizards=read.table("./Herps/cowleylizards.txt",header=T)
cowleylizards_out=stabilINLA(cowleylizards,names(cowleylizards)[-(1:3)],"Cowley County Lizards")

cowleyturtles=read.table("./Herps/cowleyturtles.txt",header=T)
cowleyturtles_out=stabilINLA(cowleyturtles,names(cowleyturtles)[-(1:3)],"Cowley County Turtles")

#Ora Banda
orabanda=read.table("./Herps/orabanda.txt",header=T)
orabanda_out=stabilINLA(orabanda,names(orabanda)[-(1:3)],"Ora Banda Herps")

orabandasnakes=read.table("./Herps/orabandasnakes.txt",header=T)
orabandasnakes_out=stabilINLA(orabandasnakes,names(orabandasnakes)[-(1:3)],"Ora Banda Snakes")

orabandalizards=read.table("./Herps/orabandalizards.txt",header=T)
orabandalizards_out=stabilINLA(orabandalizards,names(orabandalizards)[-(1:3)],"Ora Banda Lizards")
...
desertlab_out=stabilINLA(desertlab,names(desertlab)[-(1:3)],"Desert Laboratory Annuals (open habitats)")

desertlabshrub=read.table("./Plants/desertlab_shrub.txt",header=T)
desertlabshrub_out=stabilINLA(desertlabshrub,names(desertlabshrub)[-(1:3)],"Desert Laboratory Annuals (shrub habitats)")

#Portal Summer annuals
portalsummer=read.csv("./Plants/portalsummer.csv",header=T)
portalsummer_out=stabilINLA(portalsummer,names(portalsummer)[-(1:3)],"Portal Summer annuals")

#Portal Winter annuals
portalwinter=read.csv("./Plants/portalwinter.csv",header=T)
portalwinter_out=stabilINLA(portalwinter,names(portalwinter)[-(1:3)],"Portal Winter annuals")
