...
stabilINLA=function(abund, sp_names, dataname) {
...
#linear model log(Nt+1/Nt) ~ relative abundance 

  r=inla(y ~ f(w, model = "linear",
           mean.linear=0, prec.linear=.05),
     data = data.frame(y=rates[,i],w=relA[,i]), family = "gaussian",
...
results[i,]=c(sp_names[i],-r$summary.fixed[2,1]/r$summary.fixed[1,1],r$summary.fixed[1,1]) }
}
...
pattern=summary(lm(1/(-as.numeric(slope))~as.numeric(intercept),data=results2))
...
#plot data and fitted negative frequency dependence
  ...
  abline(a=-as.numeric(results[i,2])*as.numeric(results[i,3]),b=results[i,3],col=color[i],lwd=2) }
  for(i in intersect(which(results[,3]<0),which(results[,2]<0))) {
  abline(a=-as.numeric(results[i,2])*as.numeric(results[i,3]),b=results[i,3],,col=color[i],lwd=2,lty=2) }
  
  legend("topright",legend=sp_names,col=color[1:S],pch=symbol[1:S],lwd=2,cex=.75,bty='n')
  mtext(side=3,paste('pattern=',round(pattern$coefficients[2,1],3),'r_sq=',round(pattern$r.squared,3),'p-val=',round(pattern$coefficients[2,4],3),sep=" "),adj=0,line=0.5,cex=0.75)
...
#plot relationship between frequency and strength of stabilization
  ...
  abline(lm(1/(-as.numeric(slope))~as.numeric(intercept),data=results2),lwd=2,col="red")
  #identify outliers
